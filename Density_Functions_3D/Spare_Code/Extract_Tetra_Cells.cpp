...
void Extract_Tetra_Cells ( P3 const& base_pt, vector<vector<Tetrahedron>>const& tetras, vector<double>const& max_radii, int zone_limit, vector<vector<vector<Pl3>>>& tetra_cells, vector<double>& cell_volume )
{
    ...
    for (int counter_1 = 0; counter_1 < zone_limit; ++counter_1)
    {
        ...
        single_zone_tetra_cells.reserve( (int)tetras[counter_1].size() );
        
        for (int counter_2 = 0; counter_2 < tetras[counter_1].size(); ++counter_2)
        {
            double tetra_volume = to_double( tetras[counter_1][counter_2].volume() );
            
            if (tetra_volume < tiny_num * 1e4) continue;
            
            vector<P3> vertices;
            
            vertices.push_back( tetras[counter_1][counter_2].vertex( 0 ) );
            vertices.push_back( tetras[counter_1][counter_2].vertex( 1 ) );
            vertices.push_back( tetras[counter_1][counter_2].vertex( 2 ) );
            vertices.push_back( tetras[counter_1][counter_2].vertex( 3 ) );
            ...
            Sphere s = Sphere( base_pt, max_radii[counter_1] * 1.1 );
