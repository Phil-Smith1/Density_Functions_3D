...
void Write_Poly_VTK_File ( vector<B_Poly>const& polys )
{
    ...
    for (int counter = 0; counter < polys.size(); ++counter)
    {
        if (polys[counter].zone <= 10)
        {
            ...
            ofstream ofs( "/Users/philsmith/Documents/Work/Xcode Projects/Density_Functions_3D/Output/VTK/BCC_" + to_string( polys[counter].zone ) + "_" + to_string( num_polys_written ) + ".vtk" );
